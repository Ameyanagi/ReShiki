...
from engine.worker import handle, from_document, text_runs, TEXT_DEFAULTS
...
class ChemistryTests(unittest.TestCase):
    def test_atom_indicators_from_chemdraw_remain_owned_and_keep_identity(self):
        result=handle(dict(protocol=1,operation='import',format='cdxml',text=(ROOT/'tests/fixtures/atom-labels-chemdraw.cdxml').read_text()))
        doc=result['document']
        self.assertEqual(result['analysis']['formula'],'C9H11NO2')
        self.assertEqual(doc['annotations'],[])
        self.assertEqual([a['display']['number']['text'] for a in doc['atoms']],list(map(str,range(1,13))))
        ...
        self.assertEqual(float(root.get('LabelSize')),10.)
    ...
    def test_cip_is_recomputed_instead_of_trusting_imported_text(self):
        ...
        self.assertEqual([a['cip_label'] for a in result['document']['atoms'] if a['cip_label']],['S'])
        for smiles,label in [('F/C=C/F','E'),('F/C=C\\F','Z')]:
            result=imported(smiles)
            self.assertEqual([b['cip_label'] for b in result['document']['bonds'] if b['cip_label']],[label])
    ...
    def test_unknown_or_detached_object_tags_are_rejected(self):
        ...
        with self.assertRaisesRegex(ValueError,'object tag'):
            handle(dict(protocol=1,operation='import',format='cdxml',text=xml.replace('Name="number"','Name="custom"')))
    ...
    def test_worker_recovers_from_malformed_graph_and_large_integer(self):
        ...
        bad=copy.deepcopy(doc);bad['bonds'][0]['a']=999
        huge=copy.deepcopy(doc);huge['atoms'][0]['charge']=10**100
        requests=[dict(protocol=1,operation='analyze',document=bad),dict(protocol=1,operation='analyze',document=huge),good]
        proc=subprocess.run([sys.executable,str(ROOT/'engine/worker.py')],input='\n'.join(json.dumps(r) for r in requests)+'\n',capture_output=True,text=True,timeout=15)
    ...
    def test_chemdraw_saved_symbols_keep_atom_ownership_and_jacs_defaults(self):
        ...
        doc=result['document'];self.assertEqual(doc['graphics'],[])
        self.assertEqual(doc['atoms'][0]['marks'][0]['kind'],'circled_charge')
        output=handle(dict(protocol=1,operation='export',format='cdxml',document=doc))['output']
        root=ET.fromstring(output)
        self.assertEqual(float(root.get('BondLength')),14.4)
        self.assertEqual(float(root.get('LineWidth')),.6)
        self.assertEqual(float(root.get('LabelSize')),10.)
        ...
        free=handle(dict(protocol=1,operation='import',format='cdxml',text=(ROOT/'tests/fixtures/symbols-chemdraw.cdxml').read_text()))['document']
        self.assertEqual([g['kind'] for g in free['graphics']],[{'orbital':'p'},{'symbol':'circle_plus'}])
    ...
    def test_radical_identity_survives_chemical_formats_and_cleanup(self):
        for smiles,count in [('[CH3]',1),('[CH2]',2),('[NH3+]',1)]:
            with self.subTest(smiles=smiles):
                result=imported(smiles);doc=result['document']
                ...
                for fmt in ('mol','cdxml'):
                    output=handle(dict(protocol=1,operation='export',format=fmt,document=doc))['output']
        ...
        doc=imported('C')['document'];doc['atoms'][0]['radical_electrons']=1
        doc['atoms'][0]['explicit_h']=4;doc['atoms'][0]['no_implicit']=True
        with self.assertRaises(ValueError):handle(dict(protocol=1,operation='analyze',document=doc))

    def test_unsupported_attached_symbols_fail_without_silent_chemical_changes(self):
        doc=imported('C')['document'];doc['atoms'][0]['marks']=[dict(kind='lone_pair',offset={'x':0,'y':-20})]
        ...
        with self.assertRaisesRegex(ValueError,'native/SVG/PDF/PNG'):
            handle(dict(protocol=1,operation='export',format='cdxml',document=doc))
        xml=(ROOT/'tests/fixtures/symbols-chemdraw.cdxml').read_text().replace('OrbitalType="p"','OrbitalType="pShaded"')
    ...
    def test_cdxml_attached_charges_keep_the_reference_editors_proximity_contract(self):
        ...
        doc['atoms'][0]['explicit_h']=0;doc['atoms'][0]['no_implicit']=False
        doc['atoms'][0]['marks']=[dict(kind='circled_charge',offset={'x':0,'y':-10*42/14.4})]
        xml=handle(dict(protocol=1,operation='export',format='cdxml',document=doc))['output']
        node=ET.fromstring(xml).find('.//n')
        self.assertEqual(node.get('NumHydrogens'),'4')
        self.assertIsNotNone(node.find('t'))
        saved=handle(dict(protocol=1,operation='import',format='cdxml',text=(ROOT/'tests/fixtures/exported-charge-chemdraw.cdxml').read_text()))
    ...
    def test_builtin_catalog_matches_frozen_pubchem_formula_and_stereochemistry(self):
        ...
        self.assertEqual(len(catalog),79);self.assertEqual(len(library),len(catalog))
        ...
        for expected,item in zip(catalog,library):
            with self.subTest(name=expected['name']):
                ...
                for operation in ('analyze','clean'):
                    result=handle(dict(protocol=1,operation=operation,document=item['document']))
                ...
                if expected.get('sequence'):
                    mol=handle(dict(protocol=1,operation='export',format='mol',document=item['document']))['output']
    ...
    def test_chemdraw_saved_ring_tools_are_molecules_and_keep_projected_positions(self):
        result=handle(dict(protocol=1,operation='import',format='cdxml',text=(ROOT/'tests/fixtures/ring-presets-chemdraw.cdxml').read_text()))
        doc=result['document'];self.assertEqual(len(doc['atoms']),17)
        ...
        for a,b in zip(doc['atoms'],restored['document']['atoms']):
            for axis in ('x','y'):
                self.assertAlmostEqual(a['position'][axis]-doc['atoms'][0]['position'][axis],b['position'][axis]-restored['document']['atoms'][0]['position'][axis],places=3)
    ...
    def test_chemdraw_saved_arrows_keep_head_styles_and_exact_bezier_controls(self):
        ...
        self.assertEqual(equilibrium['style']['gap_pt'],2.)
        ...
        output=handle(dict(protocol=1,operation='export',format='cdxml',document=result['document']))['output']
        again=handle(dict(protocol=1,operation='import',format='cdxml',text=output))['document']
        self.assertEqual([a['style'] for a in again['arrows']],[a['style'] for a in arrows])
        for a,b in zip(arrows,again['arrows']):
            for axis in ('x','y'):
                self.assertAlmostEqual(a['end'][axis]-a['start'][axis],b['end'][axis]-b['start'][axis],places=3)
                if 'control' in a:
                    self.assertAlmostEqual(a['control'][axis]-a['start'][axis],b['control'][axis]-b['start'][axis],places=3)
    ...
    def test_cdxml_arrow_exchange_rejects_combinations_the_reference_editor_changes(self):
        ...
        doc['arrows']=[a]
        for kind,change in [('forward',dict(pattern='dotted')),('retro',{}),('equilibrium',dict(equilibrium_ratio=.5)),('curved',dict(shape='hollow')),('curved',dict(dipole=True)),('dipole',dict(tail='full'))]:
            with self.subTest(kind=kind,change=change):
                a['kind']=kind;a['style']={**appearance(dict(kind=kind)),**change}
                with self.assertRaises(ValueError):handle(dict(protocol=1,operation='export',format='cdxml',document=doc))
        xml=(ROOT/'tests/fixtures/arrows-chemdraw.cdxml').read_text()
        root=ET.fromstring(xml);root.find('.//arrow').set('AngularSize','90')
        with self.assertRaisesRegex(ValueError,'elliptical'):
            handle(dict(protocol=1,operation='import',format='cdxml',text=ET.tostring(root,encoding='unicode')))
    ...
    def test_arrow_notches_colors_and_groups_survive_exchange_and_clean(self):
        ...
        arrow['style']={**appearance(arrow), 'shape':'hollow','head_notch':.25,'color':[32,80,145],'head_length_pt':4.,'head_width_pt':2.}
        doc['arrows']=[arrow]
        ...
        xml=handle(dict(protocol=1,operation='export',format='cdxml',document=doc))['output']
        restored=handle(dict(protocol=1,operation='import',format='cdxml',text=xml))['document']
        self.assertEqual(restored['arrows'][0]['style'],arrow['style'])
        self.assertEqual(len(restored['groups']),1)
        self.assertEqual(set(restored['groups'][0]['members']),{restored['arrows'][0]['id'],restored['annotations'][0]['id']})
    ...
    def test_chemdraw_saved_extended_bond_gallery_preserves_chemistry_and_appearance(self):
        ...
        self.assertEqual([(b['order'],b['display']) for b in doc['bonds']],[(1,'plain'),(2,'plain'),(3,'plain'),(1,'wedge'),(1,'hash'),(1,'hollow_wedge'),(1,'wavy'),(1,'bold'),(5,'dashed'),(0,'dotted'),(1,'plain'),(1,'hashed'),(7,'plain'),(7,'dashed'),(2,'bold'),(2,'wavy'),(5,'plain'),(6,'plain')])
        ...
        self.assertEqual(h['element'],'H');self.assertEqual(acceptor['label_h'],3)
    ...
    def check_stereo_depiction(self, smiles, up_style):
        ...
        for bond in doc['bonds']:
            bond['display']={'wedge':up_style,'hash':'hashed'}.get(bond['display'],bond['display'])
        ...
        for operation in ('analyze','clean'):
            ...
            for atom in drawn['atoms']:atom['stereo']=None
        ...
        xml=handle(dict(protocol=1,operation='export',format='cdxml',document=doc))['output']
        self.assertEqual(handle(dict(protocol=1,operation='import',format='cdxml',text=xml))['analysis']['smiles'],initial['analysis']['smiles'])
    ...
    def test_extended_bond_orders_keep_valence_and_reject_lossy_molecular_exports(self):
        ...
        self.assertEqual(next(a for a in r['document']['atoms'] if a['element']=='N')['label_h'],3)
        ...
        oxygen=next(a for a in water['atoms'] if a['element']=='O');nitrogen=next(a for a in water['atoms'] if a['element']=='N')
        h=copy.deepcopy(oxygen);h.update(id=3,element='H',label_h=0,position={'x':oxygen['position']['x']+42,'y':oxygen['position']['y']})
        water['atoms'].append(h)
        water['bonds']=[dict(a=oxygen['id'],b=3,order=1,display='plain'),dict(a=3,b=nitrogen['id'],order=0,display='dotted')]
    ...
    def test_chemdraw_saved_groups_keep_captions_attached_to_their_molecules(self):
        ...
        for group in doc["groups"]:
            ...
            self.assertTrue(min(a["position"]["x"] for a in atoms)<labels[0]["position"]["x"]<max(a["position"]["x"] for a in atoms))
    ...
    def test_supported_legacy_graphics_import_without_a_molecule(self):
        ...
        self.assertEqual([g["kind"] for g in result["document"]["graphics"]],
    ...
    def test_unsupported_graphic_styles_and_group_objects_fail_explicitly(self):
        ...
        for content in [
            ...
            with self.subTest(content=content), self.assertRaises(ValueError):
                handle(dict(protocol=1,operation="import",format="cdxml",text=f'<CDXML><page>{content}</page></CDXML>'))
        doc={"version":4,"atoms":[],"bonds":[],"graphics":[{"id":1,"style":{"pattern":"dotted"}}]}
        path=[{"command":"move","points":{"x":0,"y":0}},{"command":"line","points":{"x":10,"y":10}}]
        with self.assertRaisesRegex(ValueError,"Dotted graphics"):
            handle(dict(protocol=1,operation="export",format="cdxml",document=doc,graphic_paths={"1":path}))
    ...
    def test_rich_text_survives_cleanup_and_cdxml_exchange(self):
        doc = imported("CCO")["document"]
        style = {**TEXT_DEFAULTS, "family": "Helvetica", "size_pt": 12.0,
        ...
        text = "α Pd/C, H2\nEtOH · 25 °C"
        doc["annotations"] = [{"id": 50, "position": {"x": 0, "y": 100}, "text": text,
                               "format": {"style": style,
                                          "spans": [{"start": 3, "end": 7, "style": emphasis}],
                                          "alignment": "center", "line_spacing": 1.5, "width_pt": 120}}]
        ...
        self.assertEqual(result["document"]["atoms"][-1]["text_style"],
                         {**emphasis, "formula": False})
    ...
    def test_cdxml_text_without_molecule_and_explicit_scripts(self):
        ...
        doc = {"version": 3, "atoms": [], "bonds": [], "arrows": [],
               "annotations": [{"id": 1, "position": {"x": 0, "y": 0}, "text": "Fe3+",
                                "format": {"style": TEXT_DEFAULTS,
    ...
    def test_chemdraw_saved_typography_uses_reserved_colors_and_bounding_box(self):
        ...
        self.assertTrue(all(a.get("text_style", TEXT_DEFAULTS)["color"] == [0, 0, 0]
