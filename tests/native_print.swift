            ...
            for scale in [1.0,0.5] {
                for rangeOnly in [false,true] {
                    ...
                    for pageIndex in 0..<result.pageCount {
                        ...
                        // Older AppKit rounds paper dimensions to whole PostScript points.
                        // Keep the independent artwork bounds check below at half-point precision.
                        try check(abs(bounds.width-size.width)<=0.5 && abs(bounds.height-size.height)<=0.5,"Paper dimensions changed: expected=\(size) actual=\(bounds.size) rotation=\(page.rotation) settings=\(operation.printInfo.paperSize)")
