...
#[test]
fn extended_smiles_matches_native_import() -> anyhow::Result<()> {
    ...
    for line in lines {
        ...
        let result = smiles::read(&case.text);
        ...
        if let Some(error) = failure {
            ...
            if mismatches == 1 {
                ...
                std::fs::write(
                    root.join("artifacts/smiles-read-mismatch.json"),
                    serde_json::to_vec_pretty(&case)?,
                )?;
                if let Ok(actual) = result {
                    std::fs::write(
                        root.join("artifacts/smiles-read-actual.json"),
    ...
    if !all_failures.is_empty() {
        std::fs::write(
            root.join("artifacts/smiles-read-failures.txt"),
            all_failures.join("\n"),
        )?;
    }
    eprintln!("Extended SMILES: {accepted} accepted, {rejected} rejected, {mismatches} mismatches");
