    ...
    def test_crossing_depth_survives_native_interchange_and_cleanup(self):
        ...
        for fmt in ("cdxml", "cdx"):
            ...
            horizontal = next(
                ...
                for b in back["bonds"]
                if abs(
                    next(a for a in back["atoms"] if a["id"] == b["a"])["position"]["y"]
                    - next(a for a in back["atoms"] if a["id"] == b["b"])["position"]["y"]
                )
                < 0.01
            )
    ...
    def test_elbow_is_editable_through_xml_and_binary_exchange(self):
        ...
        doc["arrows"] = [
            dict(
                id=20,
                kind="bent",
                start=dict(x=100, y=0),
                end=dict(x=200, y=0),
                control=dict(x=130, y=-50),
            )
        ]
        for fmt in ("cdxml", "cdx"):
                ...
                for key in ("start", "end", "control"):
                    for axis in ("x", "y"):
                        ...
                        self.assertAlmostEqual(
                            arrow[key][axis] - arrow["start"][axis],
                            doc["arrows"][0][key][axis] - 100 * (axis == "x"),
                            places=2,
                        )
