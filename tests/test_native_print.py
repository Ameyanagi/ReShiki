...
class NativePrintingTests(unittest.TestCase):
    ...
    def compile(cls, source, name):
        ...
        result = subprocess.run(['swiftc', str(cls.root / 'native/macos/PrintSupport.swift'),
                                 str(cls.root / source), '-o', str(executable)],
    ...
    def test_malformed_requests_exit_without_showing_a_dialog(self):
        ...
        for request in requests:
            with self.subTest(length=len(request)):
                result = subprocess.run([str(self.helper)], input=request, capture_output=True, timeout=10)
