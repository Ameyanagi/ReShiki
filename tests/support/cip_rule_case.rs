...
fn run(case: &Case) -> anyhow::Result<Value> {
    ...
    let rules = if case.scheme == 10 {
        Rules::single(Rule::DescriptorPair)
    } else if case.scheme == 11 {
        Rules::single(Rule::PseudoPair)
    } else if case.scheme == 12 {
        Rules::full()
    } else if (13..=18).contains(&case.scheme) {
        let rule = if matches!(case.scheme, 13 | 14 | 17) {
            Rule::DescriptorPair
        } else {
            Rule::PseudoPair
        };
        let reference = match case.scheme {
            13 | 15 => Descriptor::R,
            14 | 16 => Descriptor::S,
            _ => Descriptor::Unknown,
        };
        Rules::with_reference(rule, reference)?
    } else if case.scheme == 8 {
...
pub fn check(script: &str, artifact: &str, minimum: usize) -> anyhow::Result<()> {
    ...
    for line in lines {
        ...
        if actual != case.expected {
            ...
            if mismatches == 1 {
                ...
                std::fs::write(
                    root.join(format!("artifacts/{artifact}-first-mismatch.json")),
