...
def run(**kw):
    return handle(dict(protocol=1, **kw))


def imported(smiles):
    return run(operation="import", format="smiles", text=smiles)
...
class Abbreviations(unittest.TestCase):
    def test_endpoint_replacement_has_expected_chemical_identity(self):
        ...
        for label, smiles in expected.items():
            with self.subTest(label=label):
                ...
                self.assertEqual(
                    result["analysis"]["inchikey"], imported(smiles)["analysis"]["inchikey"]
                )
                self.assertEqual(d, old)
                g = result["document"]["abbreviations"][0]
                self.assertEqual(g["anchor"], target)
                self.assertEqual(
                    next(a for a in result["document"]["atoms"] if a["id"] == target)["position"],
                    d["atoms"][-1]["position"],
                )
        ...
        for label in PRESETS:
            with self.subTest(label=label):
                ...
                result = run(
                    ...
                    format="replace",
                    document=d,
                    selected_ids=[d["atoms"][-1]["id"]],
                    text=label,
                )
                output = run(operation="export", document=result["document"], format="cdx")[
                    "output"
                ]
    ...
    def test_invalid_attachment_and_metadata_are_rejected(self):
        ...
        with self.assertRaises(ValueError):
            run(operation="analyze", document=d)
    ...
    def test_native_binary_groups_keep_chemistry_and_labels(self):
        ...
        result = run(
            operation="import", format="cdx", text=base64.b64encode(path.read_bytes()).decode()
        )
        self.assertEqual(
            result["analysis"]["inchikey"],
            imported("COc1ccc(NC(=O)OC(C)(C)C)cc1")["analysis"]["inchikey"],
        )
        self.assertEqual({g["label"] for g in result["document"]["abbreviations"]}, {"Boc", "OMe"})
