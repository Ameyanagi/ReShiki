...
from engine.cdx_exchange import from_cdx, to_cdx, property_bytes
...
def raster(fmt='PNG', size=(12, 8)):
    ...
    for y in range(size[1]):
        for x in range(size[0]):
            pixels.putpixel((x, y), [(255,0,0,255),(0,255,0,255),(0,0,255,255),(0,0,0,0)][
...
def wrap(data, fmt='PNG', **attrs):
    ...
    ET.SubElement(page, 'embeddedobject', id='2', **{
        'BoundingBox':'10 20 82 56', fmt:data.hex(), **attrs})
...
class PictureExchangeTests(unittest.TestCase):
    ...
    def test_raw_native_property_and_extended_blob_lengths(self):
        ...
        data = (b'VjCD0100\x04\x03\x02\x01' + bytes(10)
                + struct.pack('<HIHIHI', 0x8000,0,0x8001,1,0x8009,2)
                + property_bytes(0x0204, struct.pack('<iiii',20*65536,10*65536,56*65536,82*65536))
                + property_bytes(0x0205, struct.pack('<i',30*65536))
                + property_bytes(0x0a6f,blob) + b'\0\0'*4)
        ...
        self.assertIn(property_bytes(0x0205, struct.pack('<i',30*65536)), to_cdx(ET.tostring(root,encoding='unicode')))
    ...
    def test_mixed_native_saved_picture_keeps_group_and_chemistry(self):
        ...
        doc = result['document']
        self.assertEqual((len(doc['atoms']),len(doc['bonds']),len(doc['graphics']),len(doc['groups'])),(3,2,1,1))
        self.assertEqual(set(doc['groups'][0]['members']), {a['id'] for a in doc['atoms']} | {doc['graphics'][0]['id']})
        g = doc['graphics'][0]
    ...
    def test_malformed_pixels_and_invalid_geometry_are_atomic_errors(self):
        for attrs in [{'PNG':'not-hex'},{'PNG':b'not an image'.hex()},
                      {'BoundingBox':'1 2 1 9'},{'BoundingBox':'1 nan 3 4'},
                      {'RotationAngle':'nan'},{'alpha':'2'}, {'TIFF':raster().hex(),'PNG':''}]:
        ...
        out=io.BytesIO(); Image.new('RGB',(8193,1)).save(out,format='PNG')
    ...
    def test_export_margin_accounts_for_rotated_picture_and_relative_position(self):
        ...
        doc['graphics']=[g]
        doc['groups']=[dict(id=101,members=[a['id'] for a in doc['atoms']]+[100],integral=True)]
        for fmt in ['cdxml','cdx']:
            ...
            back=response['document']; restored=back['graphics'][0]
            self.assertEqual(response['analysis']['smiles'],'CCO')
            self.assertTrue(back['groups'][0]['integral'])
            for axis in ['x','y']:
                self.assertAlmostEqual(g['origin'][axis]-doc['atoms'][0]['position'][axis],
                                       restored['origin'][axis]-back['atoms'][0]['position'][axis],places=3)
                corners=[restored['origin'][axis]+u*restored['axis_x'][axis]+v*restored['axis_y'][axis]
                         for u,v in [(0,0),(1,0),(0,1),(1,1)]]
                self.assertAlmostEqual(min(corners)*14.4/42,30,places=3)
            self.assertEqual(decode(base64.b64decode(g['picture'])).tobytes(),
