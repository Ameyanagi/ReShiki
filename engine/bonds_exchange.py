...
DISPLAY = {
    ...
    "WedgeBegin": "wedge",
    "WedgeEnd": "wedge",
    "WedgedHashBegin": "hash",
    "WedgedHashEnd": "hash",
    "HollowWedgeBegin": "hollow_wedge",
    "HollowWedgeEnd": "hollow_wedge",
    "Wavy": "wavy",
}
EXPORT = {
    "plain": "Solid",
...
def chemistry_xml(root):
    ...
    for bond in result.iter("b"):
        ...
        if bond.get("Display2") == "DottedHydrogen":
            bond.attrib.pop("Display2")
...
def read_bonds(root, parts, scale, colors):
    ...
    for mol in parts:
        ...
        for node in fragment.findall("n"):
            ...
            for atom in mol.GetAtoms():
                ...
                if (
                    ...
                    and abs(p.x * 28 - xy[0] * scale) < 0.02
                    and abs(-p.y * 28 - xy[1] * scale) < 0.02
                ):
        ...
        for el in fragment.findall("b"):
            order = {"1": 1, "2": 2, "3": 3, "1.5": 4, "hydrogen": 0, "dative": 5, "4": 6}.get(
                el.get("Order", "1")
            )
            if order == 4 and (el.get("Display") == "Dash" or el.get("Display2") == "Dash"):
                order = 7
            if order is None:
                raise ValueError("This CDXML bond order is not supported yet")
            display = el.get("Display", "Solid")
            if el.get("Display2") == "DottedHydrogen":
                order = 0
            elif order == 1 and display == "Dash":
                order = 5
            if order == 0:
                display = "Dot"
            attrs = {}
            ...
            for ancestor in reversed(ancestors):
                attrs.update(ancestor.attrib)
            color = int(attrs.get("color", "3"))
            if not 0 <= color < len(colors):
                raise ValueError("Invalid CDXML bond color")
...
def write_bond(bond, color_id):
    attrs = {
        "Display": EXPORT[bond.get("display", "plain")],
        "color": color_id(bond.get("color", [0, 0, 0])),
    }
    ...
    if bond["order"] == 0:
        attrs.update(Order="1", Display="Dash", Display2="DottedHydrogen")
    if bond["order"] == 5 and bond.get("display") == "dashed":
        attrs.update(Order="1", Display="Dash")
    if bond["order"] == 7 and bond.get("display") == "dashed":
        attrs["Display2"] = "Dash"
    return attrs
...
def write_crossings(doc, nodes, page, middle):
    ...
    for minimum, i, a, b in sorted(segments):
        ...
        for j, c, d in active:
            ...
            if budget < 0:
                break
            if {bonds[i]["a"], bonds[i]["b"]} & {bonds[j]["a"], bonds[j]["b"]}:
                continue
            ax, ay = b["x"] - a["x"], b["y"] - a["y"]
            bx, by = d["x"] - c["x"], d["y"] - c["y"]
            det = ax * by - ay * bx
            if abs(det) < 0.001:
                continue
    ...
    for element in page.iter():
        if int(element.get("Z", middle)) > middle:
            element.set("Z", str(int(element.get("Z")) + len(ordered)))
    for rank, i in enumerate(ordered):
        nodes[i].set("Z", str(middle + rank))
        if crossings[i]:
            nodes[i].set("CrossingBonds", " ".join(nodes[j].get("id") for j in crossings[i]))
