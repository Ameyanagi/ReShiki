...
def toggle(doc, selection, read, write):
    ...
    rings = [
        ...
        and all(
            mol.GetBondBetweenAtoms(a, b).GetIsAromatic()
    ...
    keys = {
        frozenset((ids[a], ids[b])) for ring in rings for a, b in zip(ring, (*ring[1:], ring[0]))
    }
    if any(
        g["members"] and selected.intersection(g["members"]) for g in doc.get("abbreviations", [])
    ):
        ...
        if frozenset((b["a"], b["b"])) in keys:
            b["order"] = (
                ...
                if show
                    ...
                    kekule.GetBondBetweenAtoms(index[b["a"]], index[b["b"]]).GetBondTypeAsDouble()
                )
            )
            b["display"] = "plain"
    ...
    for ring in mol.GetRingInfo().AtomRings():
        ...
        for a, z in zip(points, (*points[1:], points[0])):
            ...
            if length < 0.1:
                break
            t = max(
                0, min(1, ((center["x"] - a["x"]) * dx + (center["y"] - a["y"]) * dy) / length**2)
            )
            distances.append(
...
def write_circles(fragment, doc, mol, position, color_id, next_id):
    for center, radius, color in circles(doc, mol):
        ...
        ET.SubElement(
            ...
            Center3D=position(center) + " 0",
            MajorAxisEnd3D=position(major) + " 0",
            MinorAxisEnd3D=position(minor) + " 0",
    ...
    for fragment in root.iter("fragment"):
        for el in list(fragment.findall("graphic")):
            ...
            try:
                ...
                if any(
                    math.dist(center, (c["x"], c["y"])) < 0.25
                    and abs(radius - r) < max(1.0, r * 0.2)
