...
def write(root, doc, atom_ids, position, write_text, text_defaults):
    ...
    for group in doc.get("abbreviations", []):
        ...
        anchor = nodes[str(atom_ids[group["anchor"]])]
        parent = parents[anchor]
        if any(parents[nodes[i]] is not parent for i in member_ids):
            raise ValueError("Abbreviation crosses drawing fragments")
        if any(
            atoms[i].get("marks") or atoms[i].get("display", {}).get("number")
            for i in group["members"]
        ):
            raise ValueError(
                "Expand abbreviations with attached marks or atom numbers before external editable export"
            )
        internal = [b for b in parent.findall("b") if {b.get("B"), b.get("E")} <= member_ids]
        external = [
            b
            for b in parent.findall("b")
            if (b.get("B") in member_ids) != (b.get("E") in member_ids)
        ]
        if len(external) > 1:
            raise ValueError("Multiple abbreviation attachments are not supported yet")
        outer = ET.SubElement(
            parent,
            "n",
            id=str(next_id),
        ...
        next_id += 1
        inner = ET.SubElement(outer, "fragment", id=str(next_id))
        next_id += 1
        for i in group["members"]:
            node = nodes[str(atom_ids[i])]
            parent.remove(node)
            inner.append(node)
        ...
        if external:
            bond = external[0]
            side = "B" if bond.get("B") in member_ids else "E"
            other = "E" if side == "B" else "B"
            other_node = nodes.get(bond.get(other))
            if other_node is None:
                raise ValueError("Missing outside abbreviation atom")
            faces_left = (
                float(other_node.attrib["p"].split()[0])
                > float(outer.attrib["p"].split()[0]) + 0.01
            )
            connection = str(next_id)
            next_id += 1
            ET.SubElement(
                inner, "n", id=connection, p=other_node.get("p"), NodeType="ExternalConnectionPoint"
            )
            ET.SubElement(
                inner,
                "b",
        ...
        style = {
            **text_defaults,
            **(atoms[group["anchor"]].get("text_style") or {}),
            "formula": True,
            "script": "normal",
        }
        write_text(
            ...
            {"style": style},
            p=outer.get("p"),
            LabelAlignment="Right" if faces_left else "Left",
        )
...
def flatten(root):
    ...
    for outer in wrappers:
        fragments = outer.findall("fragment")
        text = outer.find("t")
        if len(fragments) != 1 or text is None:
            raise ValueError("An abbreviation needs its explicit chemical definition")
        inner = fragments[0]
        ...
        if len(connections) > 1:
            raise ValueError("Multiple abbreviation attachments are not supported yet")
        parents = {child: parent for parent in root.iter() for child in parent}
        parent = parents[outer]
        if parent.tag != "fragment":
            raise ValueError("Abbreviation outside a molecular fragment")
        external = [b for b in parent.findall("b") if outer.get("id") in (b.get("B"), b.get("E"))]
        if len(external) > 1:
            raise ValueError("Multiple abbreviation bonds are not supported yet")
        target = tuple(map(float, outer.get("p", "").split()))
        if len(target) != 2 or not all(math.isfinite(x) for x in target):
            raise ValueError("Invalid abbreviation position")
        connection_bond = None
        connection = None
        if connections:
            connection = connections[0]
            matches = [
                b for b in inner.findall("b") if connection.get("id") in (b.get("B"), b.get("E"))
            ]
            if len(matches) != 1:
                raise ValueError("Invalid abbreviation connection point")
            connection_bond = matches[0]
            anchor = nodes.get(
                connection_bond.get("E")
                if connection_bond.get("B") == connection.get("id")
                else connection_bond.get("B")
            )
            ...
            if external:
                raise ValueError("Missing abbreviation attachment point")
            anchor = next(iter(nodes.values()), None)
        if anchor is None:
            raise ValueError("Empty abbreviation definition")
        origin = tuple(map(float, anchor.get("p", "").split()))
        if len(origin) != 2:
            raise ValueError("Missing abbreviation atom position")
        rotation = 0.0
        scale = 1.0
        if connection is not None and external:
            ...
            outside_id = (
                external[0].get("E")
                if external[0].get("B") == outer.get("id")
                else external[0].get("B")
            )
            outside = next((n for n in parent.findall("n") if n.get("id") == outside_id), None)
            if outside is None:
                raise ValueError("Missing abbreviation neighbor")
            dest = tuple(map(float, outside.get("p", "").split()))
            if len(source) != 2 or len(dest) != 2:
                raise ValueError("Invalid abbreviation attachment geometry")
            source_length = math.dist(source, origin)
            dest_length = math.dist(dest, target)
            if source_length > 1e-6 and dest_length > 1e-6:
                rotation = math.atan2(dest[1] - target[1], dest[0] - target[0]) - math.atan2(
                    source[1] - origin[1], source[0] - origin[0]
                )
        ...
        for node in nodes.values():
            ...
            if len(values) != 2 or not all(math.isfinite(v) for v in values):
                raise ValueError("Invalid abbreviation atom position")
        ...
        for bond in inner.findall("b"):
            if bond is not connection_bond:
                parent.append(bond)
        ...
        for t in anchor.findall("t"):
            anchor.remove(t)
        label_text = copy.deepcopy(text)
        label_text.set("p", anchor.get("p"))
        anchor.append(label_text)
        ...
        for name, (_, back) in PRESETS.items():
            if label == name:
                reverse = back
                break
            if back and label == back:
                label, reverse = name, back
                break
        records.append(
            dict(
                label=label,
                ...
                members=[i for i, n in nodes.items() if n is not connection],
            )
        )
        parent.remove(outer)
    return records
    ...
    def identifier(xml_id):
        ...
        for atom in mol.GetAtoms():
            ...
            if (
                ...
                and abs(p.x * 28 - x * scale) < 0.02
                and abs(-p.y * 28 - y * scale) < 0.02
            ):
                candidates.append(atom.GetIdx() + 1)
        if len(candidates) != 1:
            raise ValueError("Could not safely associate abbreviation atoms")
        return candidates[0]

    return [
        dict(
            label=g["label"],
            reverse_label=g["reverse_label"],
            anchor=identifier(g["anchor"]),
            members=[identifier(i) for i in g["members"]],
        )
        for g in records
    ]
