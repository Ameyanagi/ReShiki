...
def fragments(doc):
    ...
    parts = [dict(version=doc['version'],atoms=[],bonds=[],abbreviations=[],atom_labels=doc.get('atom_labels',{})) for _ in members]
    for atom in doc['atoms']: parts[owners[atom['id']]]['atoms'].append(atom)
    for bond in doc['bonds']: parts[owners[bond['a']]]['bonds'].append(bond)
...
def clean(doc, options, selection, read, write, analyze, scale, bond_length):
    ...
    if scope not in ('selected_atoms','selected_molecules','drawing') or not isinstance(keep_orientation,bool):
    ...
    for members,base in fragments(doc):
        ...
        fixed = {a.GetIdx():Point2D(old[a.GetIdx()].x,old[a.GetIdx()].y)
                 for a in mol.GetAtoms() if int(a.GetProp('moruno_id')) not in moving}
        rdDepictor.Compute2DCoords(mol,canonOrient=False,coordMap=fixed,
                                  bondLength=bond_length/scale,forceRDKit=True,useRingTemplates=True)
        ...
        for i,p in fixed.items():
            ...
            if math.hypot(q.x-p.x,q.y-p.y)>1e-6:
                raise ValueError('Cleanup could not preserve the fixed atoms; choose Selected molecules')
        ...
        merged['bonds'] = [by_edge[frozenset((b['a'],b['b']))] if {b['a'],b['b']}.intersection(moving) else b for b in base['bonds']]
        checked = read(merged)
        ...
        for old_bond in mol.GetBonds():
            ...
            if old_bond.GetStereo() in (Chem.BondStereo.STEREONONE,Chem.BondStereo.STEREOANY) and new_bond.GetStereo() not in (Chem.BondStereo.STEREONONE,Chem.BondStereo.STEREOANY):
                a=int(old_bond.GetBeginAtom().GetProp('moruno_id'));b=int(old_bond.GetEndAtom().GetProp('moruno_id'))
                if not {a,b}.intersection(moving): continue
        ...
        if Chem.MolToSmiles(read(merged),isomericSmiles=True) != identity:
            raise ValueError('Cleanup could not preserve stereochemistry; try including the whole molecule')
        ...
        for drawing in (drawn_before,drawn_after):
            for a in drawing['atoms']: a['stereo'] = None
            for b in drawing['bonds']:
                b['stereo'] = None
                b['stereo_atoms'] = []
        if Chem.MolToSmiles(read(drawn_before),isomericSmiles=True) != Chem.MolToSmiles(read(drawn_after),isomericSmiles=True):
            raise ValueError('Cleanup could not preserve visible stereochemistry; include the whole molecule')
        updated_atoms.update((a['id'],a) for a in merged['atoms'] if a['id'] in moving)
        updated_bonds.update((frozenset((b['a'],b['b'])),b) for b in merged['bonds'] if {b['a'],b['b']}.intersection(moving))
        count += 1
    if not count:
    ...
    result['version'] = 11
    warnings = [f"{crossed} double bond(s) use a crossed depiction to keep unspecified stereochemistry."] if crossed else []
    try:
        analysis = analyze(read(result))
    except (ValueError, RuntimeError) as error:
        if scope == 'drawing': raise
        analysis = None
        warnings.append('Selected geometry cleaned. Another part of the drawing needs checking: '+str(error))
