...
ENCODINGS = {65001:'utf-8', 10000:'mac_roman', 1252:'cp1252', 0:'latin-1',
             932:'cp932', 936:'gbk', 949:'cp949', 950:'big5', 1251:'cp1251'}
...
def property_bytes(code, data):
    if len(data) > LIMIT: raise ValueError('Drawing property is too large')
...
def coordinates(value, kind, encode=True):
    ...
    if encode:
        ...
        if len(value) != 4*count: raise ValueError('Invalid binary coordinates')
        values = [v/65536 for v in struct.unpack('<'+'i'*count, value)]
    if count == 2: values = [values[1], values[0]]
    if count == 4: values = [values[1], values[0], values[3], values[2]]
...
def numeric(name, kind, enum, value, encode=True):
    ...
    if encode:
        if value in enum: number = enum[value]
        elif name in BITFIELDS and enum:
            try: number = int(value)
            except ValueError:
                number = 0
                for part in value.split():
                    if part not in enum: raise ValueError('Unsupported drawing flag: '+name)
                    number |= enum[part]
        elif enum and name not in ('LineHeight','CaptionLineHeight','LabelLineHeight'): raise ValueError('Unsupported drawing value: '+name+'='+value)
        else: number = float(value)
        if kind == 'CDXCoordinate' or name in ANGLES: number *= 65536
        if name == 'BondSpacing': number *= 10
        if kind != 'FLOAT64': number = round(number)
        if not math.isfinite(number): raise ValueError('Non-finite drawing value')
    ...
    fmt = 'H' if kind == 'INT16' and name in BITFIELDS else INTS[kind]
    if len(value) != struct.calcsize(fmt): raise ValueError('Invalid binary number: '+name)
    number = struct.unpack('<'+fmt, value)[0]
    if not math.isfinite(number): raise ValueError('Non-finite binary number')
    if name == 'DoublePosition' and number in (0,1,2): return 'auto'
    if enum:
        reverse = {v:k for k,v in enum.items()}
        if number in reverse:
            value = reverse[number]
            return value.lower() if name in ('LineHeight','CaptionLineHeight','LabelLineHeight') else value
        if name in BITFIELDS:
            if name == 'CurveType': return str(number)
            flags = [key for key,val in enum.items() if val and (number & val) == val]
            known = 0
            for key in flags: known |= enum[key]
            if known == number: return ' '.join(flags)
...
def encode_text(element):
    runs, parts, length = [], [], 0
    for span in element:
        if span.tag != 's': continue
        data = (span.text or '').replace('\n','\r').encode('utf-8')
        runs.append(packed('HHHHH', length, int(span.get('font','3')), int(span.get('face','0')),
                           round(float(span.get('size','10'))*20), int(span.get('color','3'))))
        parts.append(data)
        length += len(data)
        if length > 65535: raise ValueError('A binary text object must be shorter than 65536 UTF-8 bytes')
...
def decode_text(data, element, fonts, utf8=False):
    ...
    for i, (start,font,face,size,color) in enumerate(runs):
        ...
        try: value = text[start:offsets[i+1]].decode(encoding).replace('\r','\n')
        except UnicodeError as e: raise ValueError('Invalid binary drawing text encoding') from e
        attrs = {} if font is None else dict(font=str(font), face=str(face), size=str(size/20), color=str(color))
        ET.SubElement(element,'s',**attrs).text = value


def to_cdx(xml):
    if len(xml.encode('utf-8')) > LIMIT: raise ValueError('Drawing exceeds the 16 MB structure limit')
    root = ET.fromstring(xml)
    if root.tag != 'CDXML': raise ValueError('Expected a CDXML drawing')
    ...
    def write(el, depth=0):
        ...
        if el.tag not in tags: raise ValueError('Unsupported binary drawing object: '+el.tag)
        identifier = int(el.get('id','0'))
        if identifier == 0 and el is not root:
            identifier = next_id; next_id += 1
        result = bytearray(packed('HI', tags[el.tag], identifier))
        for name, value in el.attrib.items():
            if name == 'id': continue
            if name not in BY_NAME: raise ValueError('Unsupported binary drawing property: '+name)
            code, kind, enum = BY_NAME[name]
            if kind in INTS: data = numeric(name,kind,enum,value)
            elif kind in ('CDXBoolean','CDXBooleanImplied'):
                if value not in ('yes','no'): raise ValueError('Invalid drawing boolean')
                ...
                data = packed('H',len(points)//2)+b''.join(coordinates(' '.join(points[i:i+2]),'CDXPoint2D') for i in range(0,len(points),2))
            elif kind == 'INT16ListWithCounts':
                vals = [int(v) for v in value.split()]
                data = packed('H',len(vals))+b''.join(packed('H',v) for v in vals)
            else: raise ValueError('Unsupported binary property type: '+name)
            result += property_bytes(code,data)
        if el.tag == 't': result += property_bytes(0x700,encode_text(el))
        for child in el:
            ...
            if child.tag == 'fonttable':
                ...
                for color in child:
                    ...
                    if any(not math.isfinite(v) or not 0<=v<=1 for v in values): raise ValueError('Invalid drawing color')
                ...
                if attribute is None: raise ValueError('Unsupported represented property')
                result += property_bytes(0xe,packed('IH',int(child.attrib['object']),attribute[0]))
            else: result += write(child,depth+1)
        result += b'\0\0'
        if len(result) > LIMIT: raise ValueError('Binary drawing is too large')
        return bytes(result)
...
def from_cdx(data):
    if len(data) > LIMIT: raise ValueError('Drawing exceeds the 16 MB structure limit')
    if not data.startswith(b'VjCD0100\x04\x03\x02\x01'): raise ValueError('Invalid binary drawing header')
    reader = Reader(data)
    # Some exporters follow the 28-byte header in the prose specification.
    if data[22:24] == b'\0\x80': reader.take(22)
    elif data[28:30] == b'\0\x80': reader.take(28)
    else: raise ValueError('Binary drawing has no document object')
    ...
    def read_object(code,depth=0):
        ...
        if identifier: el.set('id',str(identifier))
        raw, children = [], []
        while True:
            tag = reader.number('H')
            if tag == 0: break
            if tag >= 0x8000: children.append(read_object(tag,depth+1));continue
            property_count += 1
            if property_count > 1_000_000: raise ValueError('Drawing property limit exceeded')
            size = reader.number('H')
            if size == 65535: size = reader.number('I')
            raw.append((tag,reader.take(size)))
        return el, raw, children
    tree = read_object(reader.number('H'))
    if tree[0].tag != 'CDXML': raise ValueError('Expected a binary document')
    ...
    def convert(tree):
        ...
        for tag,data in raw:
            if tag == 0x100:
                r=Reader(data);r.number('H');table=ET.SubElement(el,'fonttable')
                for _ in range(r.number('H')):
                    identifier,charset,length = r.number('H'),r.number('H'),r.number('H')
                    encoding=ENCODINGS.get(charset)
                    if encoding is None: raise ValueError(f'Unsupported drawing text charset {charset}')
                ...
                r.done()
        for tag,data in raw:
            if tag == 0x100: continue
            prop = PROPERTIES.get(tag)
            if prop is None:
                if el.tag in ('CDXML','page') or (el.tag=='n' and tag in NODE_BOOKKEEPING): continue
                raise ValueError(f'Unsupported binary drawing property 0x{tag:04x}')
            name,kind,enum = prop
            if tag in (0x700,0x709): texts[tag]=data;continue
            if kind in INTS: el.set(name,numeric(name,kind,enum,data,False))
            elif kind in ('CDXBoolean','CDXBooleanImplied'):
                if data not in (b'',b'\0',b'\1'): raise ValueError('Invalid drawing boolean')
                el.set(name,'no' if data==b'\0' else 'yes')
            elif kind in ('CDXPoint2D','CDXPoint3D','CDXRectangle'): el.set(name,coordinates(data,kind,False))
            elif kind=='CDXString':
                temporary=ET.Element('t');decode_text(data,temporary,fonts)
                el.set(name,''.join(s.text or '' for s in temporary))
            elif kind=='CDXColorTable':
                r=Reader(data);table=ET.SubElement(el,'colortable')
                for _ in range(r.number('H')):
                    ET.SubElement(table,'color',**{axis:str(r.number('H')/65535) for axis in ('r','g','b')})
                r.done()
            elif kind=='CDXFontStyle':
                if len(data)!=8: raise ValueError('Invalid default font style')
                font,face,size,color=struct.unpack('<HHHH',data)
                prefix='Label' if tag==0x80a else 'Caption'
                for suffix,value in [('Font',font),('Face',face),('Size',size/20),('Color',color)]:el.set(prefix+suffix,str(value))
            elif kind=='CDXCurvePoints':
                r=Reader(data);count=r.number('H')
                el.set(name,' '.join(coordinates(r.take(8),'CDXPoint2D',False) for _ in range(count)));r.done()
            elif kind in ('CDXObjectIDArray','INT16ListWithCounts'):
                r=Reader(data);count=len(data)//4 if kind=='CDXObjectIDArray' else r.number('H')
                el.set(name,' '.join(str(r.number('I' if kind=='CDXObjectIDArray' else 'H')) for _ in range(count)));r.done()
            elif kind=='CDXRepresentsProperty':
                r=Reader(data);identifier,attribute=r.number('I'),r.number('H');r.done()
                prop=PROPERTIES.get(attribute)
                if prop is None: raise ValueError('Unsupported represented property')
        ...
        for child in children: el.append(convert(child))
        return el
