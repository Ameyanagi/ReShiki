...
def carbons(attrs):
    ...
    return 'all' if terminal and internal else 'terminal' if terminal else 'internal' if internal else 'skeletal'


def read_labels(root, mol, base, scale, read_text, object_map):
    parents = {child: parent for parent in root.iter() for child in parent}
    settings = dict(carbons=carbons(root.attrib), hydrogens=not yes(root.attrib, 'HideImplicitHydrogens'),
    ...
    for node in root.iter('n'):
        ...
        for atom in mol.GetAtoms():
            ...
            if atom.GetAtomicNum() == int(node.get('Element', '6')) and abs(p.x*28-xy[0]) < .02 and abs(-p.y*28-xy[1]) < .02:
        ...
        display = dict(carbons=carbons(attrs), hydrogens=not yes(attrs, 'HideImplicitHydrogens'),
        ...
        display['hydrogen_position'] = {'Auto': 'auto', 'Best': 'auto'}.get(position, position.lower())
    ...
    for element in list(root.iter('n')) + list(root.iter('b')):
        ...
        for tag in element.findall('objecttag'):
            name = tag.get('Name')
            if name not in ('number', 'stereo') or name in seen or (name == 'number' and element.tag != 'n'):
            ...
            if box:
                ...
                item['offset'] = dict(x=(box[0]+box[2])/2-anchor[0], y=(box[1]+box[3])/2-anchor[1])
            if name == 'number':
                item['text'] = value
                target['number'] = item
            else:
...
def carbon_attributes(value):
    return dict(ShowTerminalCarbonLabels='yes' if value in ('terminal', 'all') else 'no',
...
def visible(atom, doc):
    ...
    mode = atom.get('display', {}).get('carbons') or doc.get('atom_labels', {}).get('carbons', 'skeletal')
    return (atom['element'] != 'C' or atom.get('charge') or atom.get('isotope') or atom.get('radical_electrons')
            or degree == 0 or mode == 'all' or (mode == 'terminal' and degree == 1) or (mode == 'internal' and degree > 1))


def write_labels(root, doc, object_map, bond_nodes, position, scale, write_text, defaults, indicators):
    ...
    def indicator(node, owner, kind, text, data, anchor):
        ...
        metrics = next((i for i in indicators or [] if i['owner'] == owner and i['text'] == text), None)
        style = {**defaults, 'size_pt': 7.5, 'italic': kind == 'stereo', **data.get('style', {})}
        width = len(text) * style['size_pt'] * .6 / scale
        ...
        if metrics:
            width, height, origin, center = (metrics[k] for k in ('width', 'height', 'origin', 'center'))
            style = metrics['style']
        tag = ET.SubElement(node, 'objecttag', Name=kind, TagType='Unknown')
        far = dict(x=origin['x']+width, y=origin['y']+height)
        baseline = dict(x=origin['x'], y=origin['y']+height*.9)
        write_text(tag, text, dict(style=style), p=position(baseline),
                   BoundingBox=position(origin)+' '+position(far), CaptionLineHeight='variable')
    ...
    for atom in doc['atoms']:
        ...
        if number:
            ...
            indicator(node, {'Number': atom['id']}, 'number', number['text'], number, atom['position'])
        ...
        if show and atom.get('cip_label'):
            indicator(node, {'AtomStereo': atom['id']}, 'stereo', '('+atom['cip_label']+')', data, atom['position'])
    ...
    for bond, node in zip(doc['bonds'], bond_nodes):
        ...
        if show and bond.get('cip_label'):
            anchor = {k: (atoms[bond['a']]['position'][k]+atoms[bond['b']]['position'][k])/2 for k in ('x', 'y')}
            indicator(node, {'BondStereo': [bond['a'], bond['b']]}, 'stereo', '('+bond['cip_label']+')', data, anchor)
