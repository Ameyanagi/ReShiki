...
def read_marks(root, mol, base, scale, object_map):
    ...
    for el in root.iter("graphic"):
        ...
        if len(owners) != 1 or any(
            rep.get("attribute") not in ("Charge", "Radical") for rep in reps
        ):
            raise ValueError("Unsupported chemical-symbol attachment")
        node = nodes.get(next(iter(owners)))
        if node is None:
            raise ValueError("Chemical symbol refers to a missing atom")
        ...
        for atom in mol.GetAtoms():
            ...
            if (
                ...
                matches.append(atom)
        if len(matches) != 1:
            raise ValueError("Could not safely associate a chemical symbol with its atom")
        atom = matches[0]
        identifier = atom.GetIdx() + 1
        symbol = el.get("SymbolType")
        if symbol in ("Plus", "Minus", "CirclePlus", "CircleMinus"):
            if not atom.GetFormalCharge():
                raise ValueError("A charge symbol refers to an uncharged atom")
            ...
            if atom.GetNumRadicalElectrons() != 1:
                raise ValueError("Single-electron mark conflicts with atom radical count")
            kind = "radical"
        elif symbol == "LonePair":
            kind = "radical" if atom.GetNumRadicalElectrons() == 2 else "lone_pair"
        else:
            raise ValueError("This attached CDXML symbol is not supported yet")
        box = [float(v) for v in el.attrib["BoundingBox"].split()]
        if len(box) != 4 or not all(math.isfinite(v) for v in box):
            raise ValueError("Invalid chemical symbol coordinates")
        size = math.hypot(box[0] - box[2], box[1] - box[3])
        if symbol in ("Electron", "LonePair"):
            size *= 2
        if not 0.5 <= size <= 96:
            raise ValueError("Unsupported chemical symbol size")
        ...
        mark = dict(
            kind=kind,
            offset={"x": box[0] * scale - xy[0], "y": box[1] * scale - xy[1]},
            angle=math.degrees(math.atan2(box[1] - box[3], box[0] - box[2])),
            size_pt=size,
        )
...
def write_marks(fragment, atoms, atom_ids, position, scale, first_id, default_label_size=10):
    for atom in atoms:
        for mark in atom.get("marks", []):
            ...
            radical = atom.get("radical_electrons", 0)
            if kind in ("charge", "circled_charge"):
                if not charge:
                    continue
                if abs(charge) != 1:
                    raise ValueError(
                        "Positioned multiple-charge marks need native/SVG/PDF/PNG export"
                    )
                symbol = ("Circle" if kind == "circled_charge" else "") + (
                    "Plus" if charge > 0 else "Minus"
                )
                attribute = "Charge"
            elif kind == "radical":
                if not radical:
                    continue
                symbol = "Electron" if radical == 1 else "LonePair"
                attribute = "Radical"
            elif kind == "lone_pair":
                if atom["element"] == "C":
                    raise ValueError(
                        "ChemDraw interprets a carbon lone-pair mark as a diradical; use native/SVG/PDF/PNG"
                    )
                symbol = "LonePair"
                attribute = "Radical"
            else:
                raise ValueError(
                    "Lone-pair bars and combined radical-ion marks need native/SVG/PDF/PNG export"
                )
            ...
            size = mark.get("size_pt") or label_size * 0.75
            if symbol in ("Electron", "LonePair"):
                size /= 2
            ...
            if any(
                other["id"] != atom["id"]
                and math.hypot(
                    center["x"] - other["position"]["x"], center["y"] - other["position"]["y"]
                )
                <= owner_distance
                for other in atoms
            ):
                raise ValueError(
                    "CDXML mark attachment is ambiguous near another atom; use native/SVG/PDF/PNG or reposition it"
                )
            end = {
                "x": center["x"] - math.cos(angle) * size / scale,
                "y": center["y"] - math.sin(angle) * size / scale,
            }
            el = ET.SubElement(
                fragment,
                "graphic",
                ...
                GraphicType="Symbol",
                SymbolType=symbol,
                BoundingBox=position(center) + " " + position(end),
            )
