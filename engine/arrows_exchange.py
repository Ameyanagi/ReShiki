...
def appearance(arrow):
    ...
    if kind == "equilibrium":
        s.update(head="left", tail="left", shape="open")
    if kind == "resonance":
        s.update(tail="full", shape="open")
    if kind == "retro":
        s.update(shape="open", head_width_pt=2.4, gap_pt=1.4)
    if kind == "fishhook":
        s.update(head="left")
    if kind == "dipole":
        s.update(dipole=True, shape="open")
    if kind == "no_go":
        s.update(no_go="cross")
...
def read_arrow(el, root, point, colors, identifier):
    ...
    if (
        el.get("FillType", "None") not in ("None", "Unspecified")
        or float(el.get("FadePercent", "100")) != 100
    ):
    ...
    if not 0 <= color < len(colors):
        raise ValueError("Invalid arrow color")
    ...
    if no_go not in ("none", "cross", "hash"):
        raise ValueError("Unsupported CDXML no-go mark")
    if length <= 0 or not 0 <= center <= length:
        raise ValueError("Unsupported arrowhead center size")
    s = appearance({})
    s.update(
        head=head,
        tail=tail,
        ...
        gap_pt=gap or 2.0,
        no_go=no_go,
        dipole=el.get("Dipole") == "yes",
    )
    for key, lo, hi in [
        ("width_pt", 0.1, 12),
        ("head_length_pt", 0.5, 24),
        ("head_width_pt", 0.25, 16),
        ("head_notch", 0, 0.9),
        ("gap_pt", 0.5, 16),
    ]:
        if not math.isfinite(s[key]) or not lo <= s[key] <= hi:
            raise ValueError(f"Unsupported CDXML arrow {key}")
    kind = (
        "equilibrium" if gap else ("resonance" if head != "none" and tail != "none" else "forward")
    )
    ...
    if el.tag == "curve":
        ...
        if len(values) == 18:
            ...
            def on_segment(p, a, b):
                ...
                if length < 1e-6:
                    return math.hypot(p["x"] - a["x"], p["y"] - a["y"]) < 0.15
                cross = abs((p["x"] - a["x"]) * dy - (p["y"] - a["y"]) * dx) / length
                t = ((p["x"] - a["x"]) * dx + (p["y"] - a["y"]) * dy) / (length * length)
                return cross < 0.15 and 0 <= t <= 1

            if not all(
                on_segment(p, a, b)
                for p, a, b in [
                    (points[2], start, corner),
                    (points[3], start, corner),
                    (points[5], corner, end),
                    (points[6], corner, end),
                ]
            ):
...
def write_arrow(parent, arrow, identifier, position, color_id):
    ...
    if arrow.get("kind") == "equilibrium" and (
    ...
    attrs = dict(
        id=str(identifier),
        ArrowheadHead={v: k for k, v in HEADS.items()}[s["head"]],
        ...
        LineWidth=str(s["width_pt"]),
        LineType="Dashed" if s["pattern"] == "dashed" else "Solid",
        FillType="None",
        color=color_id(s["color"]),
    )
    if s["no_go"] != "none":
        attrs["NoGo"] = s["no_go"].capitalize()
    if s["dipole"]:
        attrs["Dipole"] = "yes"
    ...
    if arrow.get("kind") == "bent":
        ...
        def lerp(a, b, t):
            return {k: a[k] + (b[k] - a[k]) * t for k in ("x", "y")}
