...
def read_graphics(root, scale, first_id, object_map=None):
    object_map = {} if object_map is None else object_map
    colors = palette(root)
    chemistry_z = [int(el.get("Z")) for el in root.iter()
    ...
    def style(el, filled=False, dashed=False, bold=False):
        ...
        width = float(el.get("BoldWidth" if bold else "LineWidth", root.get("BoldWidth" if bold else "LineWidth", "2" if bold else "0.6")))
        ...
        return {"stroke": colors[color], "fill": colors[color] if filled else None,
                "width_pt": max(0.1, width), "pattern": "dashed" if dashed else "solid"}

    def base(el, kind, origin, x, y, paint, path=None, sides="both"):
        z = int(el.get("Z", "0"))
        return {"kind": kind, "origin": origin, "axis_x": x, "axis_y": y,
                "style": paint, "sides": sides, "layer": z - middle if z < middle else z - middle + 1,
                "path": path or []}
    ...
    def curve(el):
        ...
        if (flags & ~(1 | 2 | 4 | 128)
                ...
                or any(el.get(k, "None") not in ("None", "Unspecified") for k in ("ArrowheadHead", "ArrowheadTail"))):
        ...
        paint = style(el, bool(flags & 128) or el.get("FillType") == "Solid",
                      bool(flags & 2) or el.get("LineType") == "Dashed", bool(flags & 4) or el.get("LineType") == "Bold")
    ...
    def graphic(el):
        ...
        x = {"x": abs(b["x"] - a["x"]), "y": 0}
        y = {"x": 0, "y": abs(b["y"] - a["y"])}
        if el.get("ArrowType", "NoHead") != "NoHead" or el.get("BracketUsage", "Unspecified") != "Unspecified":
            raise ValueError("Chemical brackets and legacy graphic arrows need a dedicated import path")
        if kind == 'Orbital':
            label=el.get('OrbitalType','')
            names={'s':'s','oval':'sigma','sigma':'sigma','lobe':'lobe','sp':'lobe','p':'p','sp3':'hybrid','dxy':'dxy','dz2':'dz2'}
            if 'Shaded' in label:raise ValueError('Gradient-shaded CDXML orbitals are not supported yet')
            ...
            if label.startswith('dz2'):key='dz2'
            if key not in names:raise ValueError('Unsupported CDXML orbital')
            axis={k:a[k]-b[k] for k in ('x','y')}
            value=base(el,{'orbital':names[key]},b,axis,{'x':-axis['y'],'y':axis['x']},style(el))
            value.update(phase=phase,phase_flipped=label.endswith('2') and not label=='dz2')
            return value
        if kind == 'Symbol':
            names={'CirclePlus':'circle_plus','CircleMinus':'circle_minus','Plus':'plus','Minus':'minus','Radical':'radical','Electron':'radical','LonePair':'lone_pair','ElectronPair':'lone_pair','RadicalCation':'radical_cation','RadicalAnion':'radical_anion','HDot':'hydrogen_dot','HDash':'hydrogen_dash','Attachment':'diamond'}
            key=el.get('SymbolType')
            if key not in names:raise ValueError('Unsupported CDXML chemical symbol')
            if el.get('ChemicallySignificant')=='yes' or el.get('ObjectID'):
                raise ValueError('Attached CDXML symbols require a dedicated chemical attachment importer')
            size=math.hypot(b['x']-a['x'],b['y']-a['y'])
            if size<.001:raise ValueError('Invalid CDXML symbol size')
            axis={k:a[k]-b[k] for k in ('x','y')}
            if key in ('Electron','Radical','LonePair','ElectronPair'):axis={k:v*2 for k,v in axis.items()}
            return base(el,{'symbol':names[key]},a,axis,{'x':-axis['y'],'y':axis['x']},style(el))
        if kind == "Line":
            if el.get("LineType", "Solid") not in ("Solid", "Dashed", "Bold"):
                raise ValueError("Unsupported graphic line style")
            return base(el, "line", a, {"x": b["x"]-a["x"], "y": b["y"]-a["y"]}, {"x": 0, "y": 0},
        ...
        if kind == "Rectangle":
            ...
            return base(el, "rounded_rectangle" if "RoundEdge" in flags else "rectangle", origin, x, y,
        ...
        if kind == "Bracket":
            ...
            if label not in names:
                raise ValueError("Single CDXML brackets need explicit orientation; import as curves")
            return base(el, names[label], origin, x, y, style(el))
        if kind == "Oval" and all(key in el.attrib for key in ("Center3D", "MajorAxisEnd3D", "MinorAxisEnd3D")):
            center, major, minor = [point(el.attrib[key]) for key in ("Center3D", "MajorAxisEnd3D", "MinorAxisEnd3D")]
            ...
            return base(el, "ellipse", origin, x, y, style(el, "Filled" in flags, "Dashed" in flags, "Bold" in flags))
    ...
    def group(el):
        ...
        if len(parts) == 2 and parts[0]["path"] == parts[1]["path"] and parts[0]["style"]["fill"] is not None and parts[1]["style"]["fill"] is None:
            result = parts[1]
        ...
        for el in container:
            if el.tag in ("graphic", "curve", "group", "embeddedobject"):
                if el.get("SupersededBy") or el in object_map:
                    continue
                if el.tag == "group":
                    value = group(el) if list(el) and all(c.tag == "curve" and c not in object_map for c in el) else None
                    if value is None:
                        collect(el)
                        continue
    ...
    for index, g in enumerate(ordered):
        ...
        if styled_parts is None:
            if isinstance(g.get('kind'),dict):
                raise ValueError("Scientific drawing export requires the editor's styled vector parts")
            commands = (paths or {}).get(str(g['id']))
            if commands is None: raise ValueError("CDXML graphic export requires the editor's vector geometry")
            styled_parts=[dict(commands=commands,style=g.get('style',{}))]
        parsed=[]
        for part in styled_parts:
            paint={"stroke":[0,0,0],"fill":None,"width_pt":.6,"pattern":"solid",**part['style']}
            if paint['pattern']=='dotted':
                raise ValueError('Dotted graphics are preserved in native/SVG/PDF/PNG; CDXML dotted strokes are not supported yet')
        ...
        def emit(target, points, closed, color, fill, width, dashed):
            ...
            element=ET.SubElement(target,'curve',id=str(next_id),Z=str(z),
                CurvePoints=' '.join(position(p) for p in points),CurveType=str(flags),
                Closed='yes' if closed else 'no',FillType='Solid' if fill else 'None',
                LineType='Dashed' if dashed else 'Solid',LineWidth=str(width),color=color_id(color))
            next_id+=1
        ...
        for paint,subpaths in parsed:
            for points,closed in subpaths:
                ...
                if len(parsed)>1 and paint['fill'] is not None:
                    target=ET.SubElement(parent,'group',id=str(next_id),Z=str(z));next_id+=1
                if paint['fill'] is not None: emit(target,points,closed,paint['fill'],True,0,False)
                emit(target,points,closed,paint['stroke'],False,paint['width_pt'],paint['pattern']=='dashed')
