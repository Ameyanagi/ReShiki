//! Optional development bridge to the pinned RDKit reference.
#[cfg(test)]
use super::LocalEngine;
use super::{ChemistryEngine, Request, Response};
...
enum ReferenceError {
    #[error("RDKit reference checkout has no worker at {0}")]
    MissingWorker(PathBuf),
    #[error(
        "RDKit reference interpreter was not found at {0}; set RESHIKI_REFERENCE_PYTHON or prepare the checkout's .venv"
    )]
...
fn reference_python(
    ...
    if python.is_file() {
        ...
        Err(ReferenceError::MissingInterpreter(python))
    }
}
...
impl PythonEngine {
    async fn spawn() -> Result<Worker, String> {
        ...
        let mut child = command
            ...
            .map_err(|e| {
                format!(
                    "Could not start RDKit reference worker: {e}. Set RESHIKI_REFERENCE_PYTHON to a reference interpreter, or prepare the checkout's .venv."
                )
            })?;
        Ok(Worker {
