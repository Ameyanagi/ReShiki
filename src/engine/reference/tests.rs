//! Guarded reference bridge checks, enabled with `rdkit-reference`.
use super::*;
...
async fn guarded_script(
    ...
    // These tests deliberately exercise the retained worker bridge, including
    // injected worker corruption. Default native routes are tested separately.
    let engine = LocalEngine::with_backend(PythonEngine {
...
async fn compare(
    local: &LocalEngine<PythonEngine>,
...
#[test]
fn reference_interpreter_requires_an_explicit_path_or_checkout_environment() -> anyhow::Result<()> {
    let temporary = tempfile::tempdir()?;
    let root = temporary.path();
    let explicit = root.join("chosen-python");
    assert!(matches!(
    ...
    assert!(matches!(
        ...
        Err(ReferenceError::MissingInterpreter(_))
    ));
    let directory = root.join(if cfg!(windows) {
        ".venv/Scripts"
    } else {
        ".venv/bin"
    });
    std::fs::create_dir_all(&directory)?;
    let python = directory.join(if cfg!(windows) {
        "python.exe"
    } else {
        "python"
    });
    std::fs::write(&python, b"reference fixture")?;
    assert_eq!(reference_python(root, None)?, python);
