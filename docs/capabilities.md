...
**Moruno does not yet have full feature parity.** The implemented core is usable for simple molecular drawings and basic exchange. The table records the boundary of this first version.
...
| Wavy/aromatic bonds | Partial | Model/render/import support; no dedicated palette button |
| Ring placement | Partial; desktop tested | Six-membered ring and element replacement; no general fused-ring/template system |
| Charges and isotopes | Implemented | Selection controls; charged/isotopic structures covered by chemistry tests |
| Selection, movement, deletion | Implemented; movement tested on desktop | Rectangle selection, individual and grouped translation, erase; no rotate/flip/align |
...
| Text and reaction arrows | Partial; desktop tested | Plain single-line text and one straight arrow style; no reaction semantics |
...
| InChI / InChIKey | Partial | Worker calculates both; UI exports InChI; InChI import is worker-only |
...
| SVG | Implemented; export desktop tested | Vector geometry and text; no PDF/PNG/TIFF export yet |
| Templates and named abbreviations | Planned | Four examples only; no reusable template library or nickname expansion |
...
| Rich typography, curves, brackets | Planned | Plain labels only; no chemical rich-text editor |
| Publication layout and printing | Planned | No pages, journal styles, print/PDF or layout validation |
| Native binary drawing format | Planned | No binary CDX importer/exporter |
| Clipboard drawing interchange | Planned | Only canonical SMILES copy; no object copy/cut/paste |
...
| Distribution and recovery | Incomplete | Development bundle; no standalone Python packaging, autosave or crash recovery |

## Format restrictions

Native `.moruno` is the only supported complete save format for Moruno's object model. Molecular formats intentionally represent the molecular graph, not page annotations or reaction arrows. CDXML import accepts molecular drawings only and rejects page-level text, arrows and unsupported tags to avoid silently discarding them. It does not preserve arbitrary external formatting, attributes or unsupported chemistry. A successful small-file exchange does not establish compatibility with every external document.
...
8. Native reopening exposed a file-filter problem: valid files were selected but the Open button stayed disabled. Removed that filter and successfully reopened `ui-drawn-ethanol.moruno`; all 3 atoms, 2 bonds, text and arrow reappeared.
...
Final automated checks: 10 Rust tests and 9 Python tests passed; formatting, Clippy with warnings denied, and the real Rust-to-Python engine smoke check passed.
