...
- CDXML plain/solid orbital and supported free-symbol objects import as editable Moruno objects. Gradient-shaded orbitals are rejected explicitly.
...
A Moruno-exported attached charge was opened and saved in an external drawing application, then re-imported and checked as `[NH4+]`; the saved result is retained under `tests/fixtures/`. A distant-position counterexample exposed loss of the charge/ownership link, and now causes an explicit export error. Desktop artifacts, drawings, screenshots and standalone checks are under `artifacts/symbols-qa-20260920/`. The exported PNG was visually inspected; the orbital phase and atom mark match the canvas.
