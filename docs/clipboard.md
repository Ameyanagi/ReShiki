...
- If external editable export is unavailable, Copy still provides the native ReShiki drawing, available images and a sized raster CDX picture, with a visible notice. Other drawing editors receive the picture; ReShiki recovers the editable original. Projected pi-ligand styles and hidden charge labels currently use this fallback. Image representations that fail to render are also reported.
...
A seven-atom, six-bond aromatic drawing that fails chemical validation was also copied into ReShiki, back into the source editor, and returned through the native clipboard as editable objects. Both binary captures are regression fixtures. An unsupported variable-label drawing was transferred as a picture without a paste error.
