...
At a substituted carbon, all three bond directions define the junction. The thick ring edge meets the lower edge of C–F, and the thin ring edge meets its upper edge. Each color sector runs through the shared atom. The previous correction incorrectly preserved the unsubstituted ring corner and attached C–F to its outside, leaving a pointed shoulder below the substituent.

| Before: separate ring corner                                                                     | Reference outline                                                                                                | After: three-way junction                                                                                    |
| ------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| ![Previous renderer leaves a shoulder below C–F](../images/shortcut-help/arene-color-before.png) | ![Vector reference with the branch joined to both ring edges](../images/shortcut-help/arene-color-reference.png) | ![Corrected shared junction with no shoulder or filled notch](../images/shortcut-help/arene-color-after.png) |

The before image uses commit `18bf28e`; the after image uses the corrected renderer. The reference comes from the same editable drawing pasted into an external editor and copied as a vector PDF. All three exports use identical scale and alignment around the carbon. The reference has different color management and inner-line trimming; this comparison checks the outer junction. No structure is retouched.

Tests compare the three shared corners with independently exported vector coordinates within 0.005 pt, including rotations and reversed bonds. Raster checks keep the notch above C–F transparent and verify solid coverage through 24 rotations, both bond directions and multiple colors. Mixed-color sectors share a bounded underpainting to prevent translucent seams; it follows their concave outline rather than filling their convex hull. Bond orders, coordinates and molecular properties do not change.
