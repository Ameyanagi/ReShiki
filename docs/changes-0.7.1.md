...
Version 0.7.1 improves image reconstruction of metal complexes and keeps aromatic rings intact when their bond appearance changes. The screenshots below were captured from the running macOS optimized preview using Computer Use before the version bump, so the preview's status bar still reads 0.7.0. See the [release validation record](release-0.7.1-validation.md) for build and publication status.
...
[![Source image in the conversation beside its editable Cp-star rhodium dimer reconstruction](images/assistant-updates/source-and-result.png)](images/assistant-updates/source-and-result.png)
...
[![Planar and tilted Cp-star examples with the combined atom count and formula in Properties](images/assistant-updates/cp-star-projection.png)](images/assistant-updates/cp-star-projection.png)
...
Select a ligand and use the left **3D tilt** tool. For a grouped diagram, use **Ungroup** first, then select the ring, its methyl groups, and its attachment point together. Drag vertically for X tilt or horizontally for Y tilt; hold **Shift** for 15° steps, or use the explicit X/Y step buttons. The screenshot shows an additional **X +15°** step applied to the selected right ligand while the left ligand remains planar.

[![Only the right Cp-star ligand selected in the 3D tilt tool, with X and Y step controls above the canvas](images/assistant-updates/tilt-tool.png)](images/assistant-updates/tilt-tool.png)
...
[![A selected solid wedge in Properties while both tilted Cp-star rings retain their aromatic ellipses](images/assistant-updates/aromatic-wedge.png)](images/assistant-updates/aromatic-wedge.png)
...
[![Atom Labels panel with Show charge labels unchecked, hiding charge symbols on the two Cp-star examples](images/assistant-updates/charge-labels.png)](images/assistant-updates/charge-labels.png)
...
[![Three rings showing a complete aromatic circle, a partial inner curve, and a projected partial curve without extra dashes](images/assistant-updates/ring-curves.png)](images/assistant-updates/ring-curves.png)
...
## Windows popup shadows

The release also includes [PR #18](https://github.com/Ameyanagi/ReShiki/pull/18), which fixes shadows accumulating around Windows popups. The screenshots below are the Windows comparison captured for that fix; the assistant walkthrough above was captured separately on macOS.

| Before                                                                                                                             | After                                                                                                                   |
| ---------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| [![Accumulating shadows around a Windows popup](images/windows-popup-shadows/before.png)](images/windows-popup-shadows/before.png) | [![Popup after the Windows shadow fix](images/windows-popup-shadows/after.png)](images/windows-popup-shadows/after.png) |
