...
- **Presentation**: a muted version of Jmol, preserving each element's hue while reducing perceptual chroma. Light canvases use deeper tones; dark canvases use brighter tints. Carbon, hydrogen, and bonds stay neutral unless individually recolored.
...
The drawing-style panel's **Export style…** menu offers **ReShiki style** (`.reshiki-style`) or **ChemDraw CDS** (`.cds`). Both contain label fonts and bond dimensions, without canvas theme colors, artwork or page layout. ReShiki style files preserve all supported settings exactly; CDS dimensions use ChemDraw's binary precision. Load either through **Load…**.

The canvas theme dropdown offers **Import theme…** and **Export theme…**. A `.reshiki-theme` contains both **light and dark** palettes; the canvas brightness button chooses between them. Exporting a theme never changes or exports the journal style, interface appearance, or individual object overrides. Imported themes are retained in your local library and embedded in saved drawings. Selecting a theme resets atom overrides and can be undone. Embedded custom themes require the new document format (version 16); older ReShiki builds cannot open those drawings.
