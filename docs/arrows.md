...
Arrows do not attach semantically to atoms/bonds, move with a connected atom, enforce electron accounting, map reactants to products or generate mechanisms. Arbitrary splines, circular/elliptical arcs, broad hollow arrow bodies, automatic connection routing and the unsupported CDXML combinations remain gaps. The assistant can lay out branching schemes with one shared reactant; this does not add atom-attached electron-flow semantics. See the [feature status](feature-status.md).
