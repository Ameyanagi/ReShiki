...
6. **Pure Rust engine:** ship without Python, RDKit or uv. Binary drawing conversion and formula/mass calculations now run in Rust. Next replace graph validation, hydrogen assignment and the remaining conversion/descriptor operations, then parsing, aromaticity, stereo, canonicalization and layout. Keep Python/RDKit as development references and require differential tests before switching each operation.
