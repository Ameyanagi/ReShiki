...
The model specifies roles and relationships. ReShiki measures full molecular and caption bounds, calculates positions, places captions close to their molecules and uses the active physical bond length and drawing style. Small global rotation offsets from molecular coordinates are removed before layout. Assistant rotations use quarter turns and preserve bond lengths and internal geometry. This cannot make every carbonyl upright; [issue #6](https://github.com/Ameyanagi/ReShiki/issues/6) records the orientation-control limitation found during the release walkthrough. Irregular ring geometry is retained. Figure width defaults to 540 points unless specified in the request. An oversized figure is reported for correction instead of shrinking every molecule’s bond length to fit.
