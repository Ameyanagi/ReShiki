...
let readable = [
    ...
    "public.utf8-plain-text", "com.adobe.pdf", "public.svg-image"
]
func read(_ board: NSPasteboard, imageOnly: Bool = false) throws -> Response {
    guard let item = board.pasteboardItems?.first else { return Response(representations: []) }
    var result: [Representation] = []
    var size = 0
    for name in imageOnly ? pictureTypes : readable {
