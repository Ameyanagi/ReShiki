...
pub(crate) fn upsample(
    component: &mut Components, mcu_height: usize, i: usize, _upsampler_scratch_space: &mut [i16],
    has_vertical_sample: bool, image_width: usize, image_height: usize
) -> Result<(), DecodeErrors> {
    match component.sample_ratio {
        SampleRatios::V | SampleRatios::HV => {
            ...
            */
            ...
            if i > 0 {
                ...
                crate::upsampler::compatible::sample(
                    row, row_up, row_down, dest,
                    crate::upsampler::compatible::Frame {
                        width: image_width.div_ceil(if component.sample_ratio == SampleRatios::HV { 2 } else { 1 }),
                        height: image_height.div_ceil(2),
                        row: i * (component.raw_coeff.len() / stride) - 1,
                        horizontal: component.sample_ratio == SampleRatios::HV,
                        vertical: true,
                    },
                )?;
            }
                ...
                .enumerate()
            {
                ...
                if upsample {
                    // upsample
                    crate::upsampler::compatible::sample(
                        curr_row, row_up, row_down, dest,
                        crate::upsampler::compatible::Frame {
                            width: image_width.div_ceil(if component.sample_ratio == SampleRatios::HV { 2 } else { 1 }),
                            height: image_height.div_ceil(2),
                            row: i * stop_offset + pos,
                            horizontal: component.sample_ratio == SampleRatios::HV,
                            vertical: true,
                        },
                    )?;
        ...
        SampleRatios::H => {
                ...
                .zip(dest_coeff.chunks_exact_mut(component.width_stride * 2))
            {
                ...
                // row up and row down
                crate::upsampler::compatible::sample(
                    single_row, &[], &[], output_stride,
                    crate::upsampler::compatible::Frame {
                        width: image_width.div_ceil(2), height: image_height,
                        row: 0, horizontal: true, vertical: false,
                    },
                )?;
            }
        }
