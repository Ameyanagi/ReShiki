...
def property_definition(row):
    ...
    for values in cells(page):
        if len(values) == 2 and values[0] == "CDXML Name:":
            name = values[1]
        if len(values) >= 3 and re.fullmatch(r"-?\d+|0x[0-9a-fA-F]+", values[0]):
            if values[1] and not values[1].startswith("("):
                enum[values[1]] = (
                    int(values[0], 16) if values[0].startswith("0x") else int(values[0])
                )
...
for row in re.findall(r"<tr[^>]*>(.*?)</tr>", read("TableOfProperties.htm"), re.I | re.S):
    ...
    if len(values) == 4 and values[0].startswith("0x"):
        ...
        if url:
            rows.append((int(values[0], 16), values[2], values[3], url.group(1)))
...
for code, name in [
    (0x20E, "MajorAxisEnd3D"),
    (0x20F, "MinorAxisEnd3D"),
    (0xA2F, "ArrowheadType"),
    (0xA30, "ArrowheadCenterSize"),
    (0xA31, "ArrowheadWidth"),
    (0xA35, "ArrowheadHead"),
    (0xA36, "ArrowheadTail"),
]:
